...
#     with up to 31 panels of daily NRD.  Monthly map highlights rain differences
...
calcRainStats<-function(basePath,yr,wideXtra,RG){
  ...
  ECSM_NRD<- read_csv(paste0(basePath,"/ECSM_NRD_",yr,".csv"))
  #ECSM_NRD<-rename(ECSM_NRD,ROWnum=X1)
  #ECSM_NRD<-rename(ECSM_NRD,Pixel_id=variable)
  ECSM_NRD$ROWnum = NULL
  ECSM_NRD$Annual = NULL
  ECSM_NRD[,c(names(ECSM_NRD)[length(ECSM_NRD)])]<-NULL
  ...
  col.num=0
  if( yr/4 - (yr%/%4) != 0) {
    col.num <- which(colnames(wideXtra) %!in% colnames(ECSM_NRD))
    wideXtra[,col.num]<- NULL
  }
  names(wideXtra)<-cols
  ECSM_NRD<- rbind(ECSM_NRD,wideXtra)
  ...
  # meltNRD$daily_date = as.Date(paste0(as.character(yr),'-01-01') ) + 
  #   as.numeric(gsub('day','',as.character(meltNRD$variable)))-1
  ...
  LongWdates$Pixel_id <-as.factor(LongWdates$Pixel_id)
  #
  ...
  names(Rain)<-c("Pixel_id","DAILY_DATE", "DBKEY","STATION","AGENCY","XCOORD",
                 "YCOORD","Gage","CODE","YEAR","MONTH","dist","nexradRain" )
  Rain$STATION <- as.factor(Rain$STATION)
  Rain<-filter(Rain, (Gage >= 0.01 & Gage < 20 & nexradRain >= 0.01 & nexradRain < 20) | 
                 (Gage <= 0.01 & nexradRain <= 0.01))
  Rain <- na.omit(Rain)
  
  #
  # Calculate Stats for Gage and nexradRain bias
  #
  Rain.monthly.sum<- Rain %>%
    group_by(STATION,DBKEY,MONTH) %>%
    summarize_at(vars(Gage, nexradRain), sum) %>% 
    rename(Gage.sum = Gage, nexradRain.sum = nexradRain)
  ...
  Corr_GagenexradRain<-Rain %>%
    group_by(STATION,DBKEY,MONTH) %>%
    summarize(correlation = cor(Gage, nexradRain, method = "kendall")) %>%
    drop_na() %>% 
    subset(correlation >= .75 & correlation <= 1.0)
  
  RainStats<-  merge(rainGages,merge(Corr_GagenexradRain,
  ...
  if (nrow(RainStats[is.na(RainStats$nexradRain.sum),])> 0) {
    RainStats[is.na(RainStats$nexradRain.sum),]$nexradRain.sum = 0.0
  }
  RainStats<-na.omit(RainStats)
  RainStats[RainStats$YCOORD <= SouthMost,]$nexradRain.sum<-RainStats[RainStats$YCOORD <= SouthMost,]$Gage.sum
  rainGages<- unique(RG[,c("STATION","AGENCY","XCOORD","YCOORD","DBKEY" )])
  ...
  # Filter RainStats
  #
  RainStats$bias = RainStats$Gage.sum/RainStats$nexradRain.sum
  ...
  Rain$NRD<-Rain$nexradRain*Rain$bias
  
  keepCols = c("STATION","YEAR","MONTH","XCOORD","YCOORD","Gage.obs",
               "Gage.sum","nexradRain.sum","bias")
  #--------------------------------------------------------------
...
plt1ras <- function(rainRas, rainRasName, Plot.pnts, rainTheme, atVals,clpBnds2,circles=FALSE){
    #---------------------------------------
    # Create plot panel for a single raster 
    #---------------------------------------  
    Diff.pnts<- Plot.pnts[abs(Plot.pnts$sum_Rain - Plot.pnts$sum_NRD)  >1,]
    ...
    if (circles){
      myplot= levelplot(rainRas, par.settings=rainTheme, 
          ...
          sp.points(Diff.pnts,pch=1,
                    cex=round((Diff.pnts$sum_Rain - Diff.pnts$sum_NRD) / -2,2),col = "black")
          sp.points(pnts,pch=1, 
                    cex=round((Diff.pnts$sum_Rain-Diff.pnts$sum_NRD)/2,2),col = "red")
      })
    }
		else {
...
if (names(NRD)[1]=='Pixel_id'){
  NRD<-rename(NRD,variable=Pixel_id)
...
halfPixel = 3280.84
pixSz = halfPixel*2
if (names(oneYr)[2]== 'Xcoord'){
  oneYr<-rename(oneYr,X=Xcoord)
  oneYr<-rename(oneYr,Y=Ycoord)
  oneYr<-rename(oneYr,Pixel_id=variable)
}
...
xmax = ceiling(max(oneYr[c('Y')])+halfPixel)
if (nrow(oneYr)<5398){
#  ymin = floor(min(oneYr[c('Ycoord')])-halfPixel - (24*(pixSz)))
  ymin = floor(min(oneYr[c('Y')])-halfPixel - (24*(pixSz)))
} else {
#  ymin = floor(min(oneYr[c('Ycoord')])-halfPixel )
  ymin = floor(min(oneYr[c('Y')])-halfPixel )
}
...
#oneYr<-rename(oneYr,Pixel_id=variable)
oneYr[,c(names(oneYr)[length(oneYr)])]<-NULL
oneYr$ROWnum<-NULL
dateList<-names(oneYr)[-c(1,2,3)]

#---------------------------------------------
#  If oneYr is smaller than 5398 the source
...
if (nrow(oneYr)<5398){
  ...
  names(wideXtra) <-c("Pixel_id","Xcoord","Ycoord",dateList)
  oneYr<- rbind(oneYr,wideXtra)  
}
#oneYr$Xcoord <- as.numeric(oneYr$Xcoord)
oneYr$Xcoord <- as.numeric(oneYr$X)
#oneYr$Ycoord <- as.numeric(oneYr$Ycoord)
oneYr$Ycoord <- as.numeric(oneYr$Y)
rainGages<- na.omit(rainGages)
...
nnRowCo<-as.data.frame(cbind(rainGages$DBKEY,
                             as.character(oneYr[index,]$Pixel_id),dist))
...
days<-paste0('day',seq(1,nrow(all_dates)))
NRDdates <-names(oneYr)[-c(1:3,length(names(oneYr))-1,length(names(oneYr)))]
#oneyrIndex<-cbind(all_dates,format(all_dates$dates,format='%b'),days[-c(1,length(days)-1,length(days))])
oneyrIndex<-cbind(all_dates,format(all_dates$dates,format='%b'),days,NRDdates)
names(oneyrIndex)<-c('dates','month','days','NRDdates')
oneyrIndex[] <- lapply(oneyrIndex, as.character)
monthOfDays <- oneyrIndex[oneyrIndex$month==month,]$NRDdates
...
i=1
dailyPlt=list()
gageR.pnts<-allStats[as.numeric(allStats$month) == monthNum,]

#---------------------------------------------
# Define color ramp and breaks
#---------------------------------------------
top=max(ceiling(gageR.pnts$sum_Rain))
mid = mean(ceiling(gageR.pnts$sum_Rain))
...
for (m in monthOfDays) {
  ...
  NRDras <-rasterize(oneYr, ras, m,fun=mean) 
  rainRasName =  oneyrIndex[oneyrIndex$days==m,]$dates
  dailyPlt[[i]]<-plt1ras (NRDras, paste(rainRasName, 'NRD'), MonthlyRain.pnts, rainTheme, atVals,clpBnds2)
}
...
# ------------------------------------------   
fileName = paste(outPath,"/dailyNRDrain_",yr,month,".png",sep="")
...
#---------------------------------------------
for (x in seq(2, i)){
  fileName = paste(outPath,"/DayByDay/dailyNRDrain_",yr,month,"-",x-1,".png",sep="")
  trellis.device(device="png", filename=fileName,width=1600,height=2000,units="px",res=300)
  print(dailyPlt[[x]])
  dev.off()
}

fileName = paste(outPath,"/DayByDay/NRDrain_",yr,month,".png",sep="")
trellis.device(device="png", filename=fileName,width=1600,height=2000,units="px",res=300)
print(dailyPlt[[1]])
dev.off()

gageRaindf<-dcast(Rain[as.numeric(Rain$MONTH)==monthNum,],
                Pixel_id+DBKEY+STATION+AGENCY+XCOORD+YCOORD ~ DAILY_DATE,
                value.var="Gage",mean)
